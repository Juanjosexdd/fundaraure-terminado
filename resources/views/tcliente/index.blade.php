    ...
    <div class="container-fluid">
        ...
        <div class="card rounded elevation-2">
            <div class="card-header margin-bottom-0">
                <div class="row">
                    <h5 class="card-title">&nbsp;&nbsp;<i class="fa fa-list text-success"></i> Listado tipo cliente</h5>
                    @can('tcliente.create')
                    <a class="btn btn-outline-success elevation-2 btn-md ml-auto rounded float-right" data-toggle="tooltip" data-placement="left" title="Agregar tipo cliente" type="button" href="{{ route('tcliente.create')}}">
            ...
            <div class="card-body">
                <div class="form-group row">
                    <div class="col-md-6">
                    {!!Form::open(array('url'=>'tcliente','method'=>'GET','autocomplete'=>'off','role'=>'search'))!!}
                    ...
                    <tbody style="font-size: 12px;">
                       @foreach($tclientes as $tcliente)
                        <tr>
                            ...
                            @can('tcliente.edit')
                            <td>
                                <a title="Editar" href="{{route('tcliente.edit', $tcliente->id)}}" data-toggle="tooltip" data-placement="top" title="Editar" class="btn text-warning border-1 elevation-2 rounded btn-group" style="height: 32px; padding-bottom: 3px; border-right:0px; padding-top: 3px;"><i class="fa fa-edit fa-2x"></i></a>
                            </td>
                            @endcan
                            @can('tcliente.destroy')
                            <td>
                               @if($tcliente->estatus)
                                <button type="button" class="btn elevation-2 btn-danger btn-sm rounded" data-id="{{$tcliente->id}}" data-toggle="modal" data-target="#cambiarEstado">
                                    <i class="fa fa-times fa-1x"></i> Desactivar
                                </button>
                                @else
                                <button type="button" class="btn elevation-2 btn-success btn-sm rounded" data-id="{{$tcliente->id}}" data-toggle="modal" data-target="#cambiarEstado">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg rounded" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('tcliente.destroy','test')}}" method="post" class="form-horizontal">
                        {{method_field('delete')}}
                        {{csrf_field()}}
